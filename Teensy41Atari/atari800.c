...
int usbarrowup;    // 82 key218 Ctrl L 134, R 135, D 143, U 142
...
static void INPUT_Frame(void)
{
  ...
  POKEY_SKSTAT |= 0xc;
  if (INPUT_key_shift) POKEY_SKSTAT &= ~8;                                 // INPUT_key_shift=0, never used POKEY_SKSTAT set b3=0
                                                                           // b3 indicates a change in shift key
  if (INPUT_key_code < 0) last_key_code = AKEY_NONE;                       //  
                                                                           // AKEY_SHFT 0x40 AKEY_CTRL 0x80 AKEY_SHFTCTRL 0xc0
  if ((INPUT_key_code&~0x17) == AKEY_SHFTCTRL) INPUT_key_code = AKEY_NONE; // 0001 0111 not=1110 1000 and keycode 1100 0000 to 110x 0xxx == 1100 0000
                                                                           //               110x 0xxx => keycode range 0xc0-0xc7 0xd0-0xd7
                                                                           //               --------- => keycode range  192-199   208-215
                                                                           //               1100 0000                    
  if (INPUT_key_code >= 0) 
     { POKEY_SKSTAT &= ~4;                                                 // 4=0000 0100 not4=1111 1011 POKEY_SKSTAT set b2 = 0
       if ((INPUT_key_code ^ last_key_code) & ~AKEY_SHFTCTRL)              // 0xc0 192 1100 0000 ^ XOR
          { 
            ...
            POKEY_KBCODE = (UBYTE) INPUT_key_code;                         // UBYTE unsigned char
            if (POKEY_IRQEN & 0x40) 
               { if (POKEY_IRQST & 0x40) 
                    { POKEY_IRQST &= ~0x40;
                      CPU_GenerateIRQ();                                   // go POKEY
                    } else POKEY_SKSTAT &= ~0x40;                          // else keyboard over-run 
...
void at8_Step(void)
{
  ...
  // RESET (current only break) both keys here
  if ((INPUT_key_code==202) && (usbF9==0))  // Break not reset
     { digitalWrite(led15, HIGH); 
       if (POKEY_IRQEN & 0x80) 
          { POKEY_IRQST &= ~0x80;
            CPU_GenerateIRQ();
  ...
  { usbF9 = 0;  
    ...
    //////////////////////////////////////////////////////// disable alarm
    SNVS_LPCR &= ~0x02;
    while (SNVS_LPCR & 0x02); __disable_irq();
    //////////////////////////////////////////////////////// get Time:
    uint32_t lsb, msb;
    do { msb = SNVS_LPSRTCMR; lsb = SNVS_LPSRTCLR;
       } while ( (SNVS_LPSRTCLR != lsb) | (SNVS_LPSRTCMR != msb) );
    uint32_t secs = (msb << 17) | (lsb >> 15);
    /////////////////////////////////////////////////////// set alarm
    secs += 2;
    SNVS_LPTAR = secs;
    while (SNVS_LPTAR != secs);  SNVS_LPCR = tmp | 0x02; // restore control register and set alarm
    while (!(SNVS_LPCR & 0x02)); SNVS_LPCR |= (1 << 6); // turn off power
    while (1) asm("wfi");  
#else
