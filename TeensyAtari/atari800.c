...
static void INPUT_Frame(void)
{
  ...
  // //if ((usbF6>0) || (usbF7>0) || (usbF8>0) || (usbF9>0)) i = 1; // not ok here for F6-F9 genrates repeated resets
  // if ((INPUT_key_code>198) && (INPUT_key_code<203)) i = 1;     // ok here for F6-F9 only one reset/break
  // if (i==1) 
  //   { if (POKEY_IRQEN & 0x80) 
  //       { POKEY_IRQST &= ~0x80;
  //          CPU_GenerateIRQ();
...
void at8_Step(void)
{
  ...
  // OPTION
  if ((j & MASK_KEY_USER3) || (INPUT_key_code==200))
